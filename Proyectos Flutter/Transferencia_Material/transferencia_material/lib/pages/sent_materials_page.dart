// ignore_for_file: avoid_print, must_be_immutable, use_key_in_widget_constructors, unused_element, library_private_types_in_public_api, deprecated_member_use

import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:firebase_auth/firebase_auth.dart';
import 'package:flutter/material.dart';
import 'package:transferencia_material/pages/formulario_materiales.dart';
import 'package:transferencia_material/pages/re-agendar.dart';
import 'package:intl/intl.dart';
import 'package:url_launcher/url_launcher.dart'; // Importa la librería para formateo de fechas

class SendMaterial extends StatefulWidget {
  const SendMaterial({Key? key}) : super(key: key);

  @override
  _SendMaterialState createState() => _SendMaterialState();
}

class _SendMaterialState extends State<SendMaterial> {
  Stream<QuerySnapshot> getPostsStream() {
    User? user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      final postsStream = FirebaseFirestore.instance
          .collection('tasks') // Change 'formularios' to 'tasks'
  ...
  Future<void> _usersReadData(String userEmail, String mensaje) async {
    try {
                  ...
                  children: [
                    ElevatedButton(
                      onPressed: () {
                        ...
                        // Agrega aquí la navegación a la página de edición
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => EditarFormularioMaterialPage(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Materiales Enviados'),
        centerTitle: true,
      ),
      body: StreamBuilder(
