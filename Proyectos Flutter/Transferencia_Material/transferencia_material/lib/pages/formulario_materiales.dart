...
class MaterialInfo {
  ...
  List<List<int>> get compressedImages => _compressedImages;
  ...
  MaterialInfo({
    ...
    required List<List<int>>
        compressedImages, // Asegúrate de tener este parámetro
  }) : _compressedImages = List.from(
            compressedImages); // Asegúrate de inicializarlo correctamente
}
...
// Lista para mapear las opciones principales
  ...
  Future<void> _enviarFormulario() async {
    ...
    for (var material in listaMateriales) {
      ...
      // Save each image for the current material
      for (var imageBytes in material.compressedImages) {
        Uint8List imageBytesUint8 = Uint8List.fromList(imageBytes);

        String imageUrl = await _uploadImageToFirebaseStorage(imageBytesUint8);

        imageUrls.add(imageUrl);
      }

      // Save material data with associated image URLs
  ...
  void _agregarMaterial() {
    MaterialInfo nuevoMaterial = MaterialInfo(
      ...
      compressedImages: List.from(_compressedImages), // Aquí está la corrección
    );
